...
- **`remain-on-exit` is owned solely by `ensure_live_pane_stays_up`**, which arms it and then always disarms it or kills the session. It used to be armed for every Cursor launch inside `start_live_session`, but only the resume path ran the watcher that turns it back off, so a new Cursor session that the user ended with `/exit` sat there as a dead pane: tmux kept the session alive, the client stayed attached, and the only way out was to detach and press `x`. Anything else that arms it must guarantee the same disarm.
