...
- **Resume anywhere** — resume a session in tmux (`Enter`) or directly in the foreground (`Shift+Enter`). Cursor interactive resume depends on the Cursor Agent CLI: if `agent --resume` flashes and exits (a known CLI bug on some builds), ccsm shows a popover explaining the failure is outside this app's control instead of vanishing; headless `agent --resume <id> -p "…"` still works, and a second interactive `agent` in another terminal can interfere
